      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
        <div className="lg:col-span-5 space-y-6">
          ...
          <p className="font-body text-base md:text-lg leading-relaxed text-foreground/70 max-w-lg animate-fade-up-delay-2">
            Cold calling is not. Simple onboarding. Then meetings start landing. Your outbound strategy, execution, and optimization, completely done for you.
          </p>
...
const roadmap = [
  {
    ...
    desc: "You fill a form. We execute. We warm up inboxes and prep infrastructure while collecting the context we need from you.",
    tag: "Kickoff",
  },
  {
    day: "Day 2-3",
    ...
    desc: "Targeting, copy, deliverability, qualification. Our team sets everything up with zero input required from your side.",
    tag: "Execution",
  },
  {
    day: "Day 5",
    ...
    desc: "You review the copy and targeting. Want changes? We make them. Once approved, the campaign goes live immediately.",
    tag: "Optimization",
  },
  {
    day: "Day 7",
    icon: Zap,
    title: "Meetings Start Landing",
    desc: "Your calendar starts filling with qualified calls. No tools to learn or dashboards to babysit.",
    tag: "Velocity",
  },
];
    ...
    <section id="how-it-works" data-testid="how-it-works-section" className="py-20 lg:py-28 px-6 lg:px-12 bg-secondary">
      <div className="max-w-7xl mx-auto">
        <div className="mb-16 max-w-2xl">
          <span className="font-mono text-xs tracking-[0.2em] uppercase text-muted-foreground block mb-4">The Roadmap</span>
          <h2 className="font-heading text-3xl md:text-4xl lg:text-5xl font-semibold tracking-tight">
            How we scale your pipeline
            <br />
            <span className="text-foreground/50">in 7 days.</span>
          </h2>
          <p className="font-body text-base md:text-lg leading-relaxed text-foreground/60 mt-4">
            From zero to booked meetings in a single week. Our team manages the entire infrastructure and execution so you can stay focused on closing.
          </p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-0 border border-border">
          {roadmap.map((s) => (
            <div key={s.day} data-testid={`step-card-${s.day}`} className="p-8 lg:p-10 bg-white border border-border relative group hover:-translate-y-0.5 transition-transform duration-200">
              <span className="font-mono text-[10px] tracking-wider uppercase text-muted-foreground absolute top-4 right-4">{s.day}</span>
...
function ProblemSolutionSection() {
  const problems = [
    { title: "SDR teams are expensive", desc: "A single SDR costs $60-80K + commission and overhead. A team can cost millions before any tooling." },
    { title: "High turnover", desc: "Average SDR tenure is just 14 months. By the time they ramp, they're already thinking about their next move." },
    { title: "Inconsistent quality", desc: "Every SDR interprets messaging differently. AI follows brand guidelines precisely, every time." },
    { title: "Limited scalability", desc: "Each human can manage 50-100 accounts. Scaling to 1,000 requires 10-20 more hires." },
  ];

  return (
    <section className="py-20 lg:py-28 px-6 lg:px-12 bg-white border-y border-border">
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
          <div className="lg:col-span-12">
             <span className="font-mono text-xs tracking-[0.2em] uppercase text-muted-foreground block mb-4">The Traditional Sales Agency Problem</span>
             <h2 className="font-heading text-3xl md:text-4xl lg:text-5xl font-semibold tracking-tight max-w-3xl">
               Traditional SDR models are broken.
               <br />
               <span className="text-foreground/50">The model has fundamental scaling limitations.</span>
             </h2>
        ...
        <div className="mt-20 pt-20 border-t border-border">
           <span className="font-mono text-xs tracking-[0.2em] uppercase text-muted-foreground block mb-4">The Economic Shift</span>
           <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
             <div>
               <h2 className="font-heading text-3xl md:text-4xl font-semibold tracking-tight">Scale through compute,<br />not headcount.</h2>
               <p className="font-body text-base text-foreground/60 mt-4 max-w-lg">
                 The AI-native sales agency model flips the economics entirely. Primary costs are AI compute and a small oversight team, achieving margins unheard of in services.
               </p>

               <div className="grid grid-cols-2 gap-8 mt-10">
                 <div className="p-6 bg-secondary border border-border">
                   <p className="font-mono text-[10px] uppercase text-muted-foreground mb-1">Traditional Agency</p>
                   <p className="font-heading text-4xl font-bold tracking-tighter">25%</p>
                   <p className="font-body text-xs text-foreground/60 mt-2">Gross Margins</p>
                   <p className="font-mono text-[9px] text-foreground/40 mt-4">$150-200K revenue / employee</p>
                 </div>
                 <div className="p-6 bg-[#1B6B3A]/5 border border-[#1B6B3A]/20">
                   <p className="font-mono text-[10px] uppercase text-[#1B6B3A] mb-1">AI-Native Agency</p>
                   <p className="font-heading text-4xl font-bold tracking-tighter text-[#1B6B3A]">75%</p>
                   <p className="font-body text-xs text-[#1B6B3A]/70 mt-2">Gross Margins</p>
                   <p className="font-mono text-[9px] text-[#1B6B3A]/50 mt-4">$400-800K revenue / employee</p>
                 </div>
               </div>
             </div>
             ...
             <div className="space-y-6">
                <div className="bg-secondary p-8 border border-border">
                  <h4 className="font-heading text-xl font-medium mb-4 flex items-center gap-2">
                    <CheckCircle2 className="text-[#1B6B3A]" size={20} /> Continuous Learning
                  </h4>
                  <p className="font-body text-sm text-foreground/70 leading-relaxed">
                    Every email open, reply, and meeting booked feeds back into the system. The AI gets smarter with every interaction, creating a data flywheel that compounds over time.
                  </p>
                </div>
                <div className="bg-secondary p-8 border border-border">
                  <h4 className="font-heading text-xl font-medium mb-4 flex items-center gap-2">
                    <CheckCircle2 className="text-[#1B6B3A]" size={20} /> 24/7 Global Operation
                  </h4>
                  <p className="font-body text-sm text-foreground/70 leading-relaxed">
                    AI systems do not sleep, take PTO, or lose motivation. Prospects in Tokyo get the same quality engagement as New York, at their preferred local time.
                  </p>
                </div>
             </div>
           </div>
...
const faqItems = [
  ...
  },
  {
    ...
    a: "Most teams see their first AI-booked meetings within the first 72 hours. The system continuously improves. By month two, you'll see a compounding data flywheel effect on reply rates and meeting conversions.",
  },
];
